...
$plugin_installed_sql_res = mysqli_query($link, $plugin_installed_sql);
$allowed_pages = array();
$navbar_array = array();
while ($plugin_row = mysqli_fetch_assoc($plugin_installed_sql_res)){
   # include "$plugin_dir";
...
#    $allowed_pages[] = "admin";
    ...
    $admin_plugins_res = mysqli_query($link, $admin_plugins_sql);
    while ($admin_plugins_row = mysqli_fetch_assoc($admin_plugins_res)){
...
if (!in_array($requested_page, $allowed_pages)){
    ...
    $url_res = mysqli_query($link, $url_sql);
    if (mysqli_num_rows($url_res) == 0){
        include 'inc/404_header.inc.php';
        include 'inc/navbar.inc.php';
        include 'inc/404_body.inc.php';
        include 'inc/404_footer.inc.php';
        ...
        while ($url_row = mysqli_fetch_assoc($url_res)){
